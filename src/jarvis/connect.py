...
1. 개별 작업을 시작하기 전에 `jarvis_context` 를 호출한다.
   - project="{project}" (또는 repo 에 git remote URL)
   - 돌아온 컨텍스트는 이 프로젝트에서 축적·검증된 기록이다. 같은 것을 다시
     조사하지 말고 거기서 시작하되, 고정된 정답이 아니라 계속 갱신되는 기록이다.
     ⟨확인 필요⟩·⟨오래됨⟩·⟨미확정⟩ 표시(또는 `trust_notes`)가 붙은 항목은 사실로
     단정하지 말고 쓰기 전에 확인한다. 표시가 없으면 확립된 것으로 본다.
...
2. 작업 중 새로 확정된 것은 즉시 `jarvis_remember` 로 남긴다.
   ...
   - 이전에 맞다고 기록된 것이 틀린 것으로 드러나면, 같은 title 로 바로잡은 내용을
     남긴다 — 이전 것을 자동으로 대체하고 이력은 보관된다. 정답지가 스스로 갱신된다.
...
1. 개별 작업을 시작하기 전에 실행한다:
   ...
   - 출력된 컨텍스트는 이 프로젝트에서 축적·검증된 기록이다. 다시 조사하지 않되,
     ⟨확인 필요⟩·⟨오래됨⟩ 표시가 붙은 항목은 사실로 단정하지 말고 먼저 확인한다.
...
2. 작업 중 새로 확정된 규칙·명령·함정은 즉시 남긴다:
   ...
   같은 지식은 항상 같은 제목으로 (한 파일에 누적된다). 이전 기록이 틀린 것으로
   드러나면 같은 제목으로 바로잡으면 이전 것을 자동 대체한다 (이력 보관).
...
- 프롬프트에 [MyViking] 블록이 주입되면 그 내용은 이 프로젝트에서 축적·검증된
  기록이다. 같은 것을 다시 조사하지 말고 거기서 시작하고, ⚠ 주의 항목을
  거스르는 제안은 하지 않는다. 단 ⟨확인 필요⟩·⟨오래됨⟩ 표시가 붙은 항목은
  고정된 정답이 아니라 갱신 중인 기록이니 쓰기 전에 확인한다.
- 작업 중 새로 확정된 규칙·명령·결정·함정은 즉시 `jarvis_remember` 로 남긴다.
  ...
  - 이전에 맞다던 것이 틀린 것으로 드러나면 같은 title 로 바로잡는다 —
    이전 것을 자동 대체하고 이력은 보관된다 (정답지가 스스로 갱신됨).
