...
echo "  2) 에이전트 연동 설정 받기 (MCP 설정 + 자동 캡처 훅)"
echo "     ${COMPOSE[*]} exec myviking jv agent config --client claude-code \\"
echo "       --url $URL${KEY:+ --key $KEY}"
echo "     ⮑ '자동으로' 기록되게 하는 건 훅입니다. 에이전트가 도는 머신의 저장소에서:"
echo "         jv agent hooks --install --url $URL${KEY:+ --key $KEY}"
echo "       (my-viking 코어가 그 머신에 설치돼 있어야 합니다: pip install my-viking)"
echo "       설치 확인:  jv agent hooks --check --url $URL${KEY:+ --key $KEY}"
