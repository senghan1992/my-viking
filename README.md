...
**공유기·방화벽·LB 의 8787 → 이 머신 포워딩은 운영자가 알아서** 하세요. 포트·
바인딩을 바꾸려면 `deploy/.env` 의 `MYVIKING_PORT`/`MYVIKING_BIND` (아래
[Docker 로 띄우기](#docker-로-띄우기-권장)). 자동
...
됩니다. 포트·바인딩은 **`deploy/.env` 의 `MYVIKING_PORT` / `MYVIKING_BIND`** 로
바꿉니다 — `cp .env.example .env` 후 편집하고 `docker compose up -d` 를 다시
실행하면 반영됩니다. 예: `MYVIKING_PORT=9000`, `MYVIKING_BIND=127.0.0.1`(이 머신만)
또는 `100.64.0.5`(VPN IP 만). 컨테이너 안 포트는 항상 8787 입니다.
...
**웹 대시보드 → '모델' 탭에서도 같은 설정을 할 수 있습니다** — Anthropic·OpenAI·
Grok·DeepSeek·Gemini·Volcengine·Ollama 에서 고르고, 키를 넣고, '연결 테스트' 로
확인 후 저장하면 서버 설정 파일(0600)에 남습니다. Custom(OpenAI 호환)을 고르면
base URL·요청 경로·키를 직접 채울 수 있어 회사 게이트웨이나 Databricks Serving
엔드포인트도 그대로 붙습니다. 웹에서 정한 값은 환경변수보다 우선하고, '웹 설정
초기화' 를 누르면 환경변수(또는 기본값) 동작으로 돌아갑니다.
